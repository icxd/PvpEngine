...
public class GMCCommand implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        if (sender instanceof Player) {
            ...
            if (player.hasPermission("pvpengine.gamemode.creative") || player.isOp()) {
                try {
                    if (args.length != 1) {
                        player.setGameMode(GameMode.CREATIVE);
                        player.sendMessage(String.format(ConfigUtil.getFormattedString("messages.gamemode-change"), GameMode.CREATIVE.name()));
                    } else {
                        Player target = Bukkit.getPlayer(args[0]);
                        target.setGameMode(GameMode.CREATIVE);
                        player.sendMessage(String.format(ConfigUtil.getFormattedString("messages.gamemode-change-other"), target.getName(), GameMode.CREATIVE.name()));
                    }
